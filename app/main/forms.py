...
class AddIncidentForm(Form):
    ...
    age = IntegerField('Age:', 
        validators=[
            validators.required(message="Age must be a valid number"), 
    ...
    armed_comment = StringField('Additional details:', 
        validators=[
            validators.optional(), 
            validators.length(
                message=("Details about whether the deceased was armed must be"
        ...
        validators=[
            ...
            validators.length(
                message=("Details about the charges must be less than 200 "
                    "characters"), 
                max=200)])
    police_dept = SelectField(("Name of the relevant law enforcement "
        "organization:"))
    ...
    mental_health_comment = StringField('Additional details:', 
        validators=[
            validators.optional(), 
            validators.length(
                message=("Detailed mental health information must be less than"
                    " 200 characters"), 
                max=200)])
    ...
    province = SelectField("Province:", 
        choices=format_form_choices(provinces_table))
    ...
    media_source = StringField('Media source:', 
        validators=[
            validators.optional(), 
            validators.url(
                message="Media source must be a valid url"), 
            validators.length(
    ...
    image_of = StringField('Image of the deceased:', 
        validators=[
            validators.optional(), 
            validators.url(
                message="Image source must be a valid url"), 
            validators.length(
    ...
    occured_date = DateField("Date of incident:", 
        format='%d/%m/%Y',
        validators=[
            validators.required(message="Date of occurence is required")])
